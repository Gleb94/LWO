...
function removeColorClicked() {
    ...
    let ColorClicked = checkIfColorClickdAdded()
    if (ColorClicked === true) {
        for (let i = 1; i < htmlElements.selectColor.length; i++) {
            if (htmlElements.selectColor[i].value === color) {
                htmlElements.selectColor.removeColor(htmlElements.selectColor[i]);
...
function removeSelectedColorClicked() {
    ...
    function getSelectedValue(option) {
        ...
        if (i = 0) {
            return ('Please choose a value to remove');
        }
        if (i != 1) {
            return removeColor
        }
...
function addColor(color) {
    ...
    var element = document.createElement(tagName, [option]);
